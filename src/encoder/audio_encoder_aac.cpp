    ...
    , callback_(nullptr)
{
    ...
    swr_ctx_ = swr_alloc_set_opts(nullptr,
        codec_context_->channel_layout, AV_SAMPLE_FMT_FLTP, sample_rate_,
        codec_context_->channel_layout, AV_SAMPLE_FMT_FLT, sample_rate_,
        0, nullptr);
...
void AudioEncoderAAC::UpdateAdtsHeader(AVCodecContext *codec_context, int aac_length)
{
    ...
    adts_header_.get()[2] = ((codec_context->profile - 1) << 6) + (sampling_frequency_index << 2) + (channel_config >> 2);
